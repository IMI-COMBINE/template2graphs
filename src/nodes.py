...
def add_nodes(
    ...
    if 'BACTERIAL_STRAIN_SITE_REF' not in df.columns:
        for bact_annotation in df['BACTERIAL_STRAIN_NAME_annotation'].values:
    ...
    # Compound
    if 'EXT_CPD_ID' not in df.columns:
        for compound_idx in df['CPD_ID'].values:
    ...
    # Batch
    if 'EXT_BATCH_ID' not in df.columns:
        for batch_idx in df['BATCH_ID'].values:
    ...
    # Experiment
    if ('No of replicates', 'MEDIUM_annotation') not in df.columns:
        for study_id, experiment_id, experiment_date, experiment_protocol, experiment_ctrl in df[
            ['STUDYID', 'EXPID', 'EXPERIMENT_DATE', 'PROTOCOL_NAME', 'CONTROL_GROUP']
        ].values:
        ...
        for study_id, experiment_id, experiment_date, experiment_protocol, replicate_num, experiment_ctrl, experiment_medium in df[
            ['STUDYID', 'EXPID', 'EXPERIMENT_DATE', 'PROTOCOL_NAME', 'No of replicates', 'CONTROL_GROUP',
             'MEDIUM_annotation']
        ].values:
