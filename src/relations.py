...
def add_relations(
    ...
    for rows in tqdm(
        ...
        if pd.notna(animal_number) and pd.notna(result_type):
            ...
            if pd.notna(method_dict):
                ...
                method_dict = {f"statistical_{i}": j for i, j in method_dict.items()}
        ...
        if pd.notna(exp_id) and pd.notna(result_type):
            ...
            if pd.notna(method_dict):
                if isinstance(method_dict, str) and method_dict != "":
                    method_dict = ast.literal_eval(method_dict.replace("null", "None"))
                method_dict = {f"statistical_{i}": j for i, j in method_dict.items()}
                annotation.update(method_dict)
    ...
    for rows in tqdm(
        invitro_df[COLS_invitro_df].values, desc="Populating graph with in-vitro data"
    ):
        (
            exp_id,
            cpd_id,
