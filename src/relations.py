...
def _create_relation(
    ...
    tx.create(Relationship(entity_a, relation_type, entity_b, **rel_props))
...
def add_relations(
    ...
    COLS_invivo_df = [
        ...
        "RELATIVE_TIMEPOINT",
        "GROUP_DESCRIPTION",
        "PRETREATMENT_BATCH_ID",
        "PRETREATMENT_CPD_ID",
        "PRETREATMENT_DOSE",
        "PRETREATMENT_DOSING_INFO",
        "PRETREATMENT_ROUTE_OF_ADMINSTRATION",
        "INFECTION_ROUTE_annotation",
        "ROUTE_OF_ADMINISTRATION_annotation",
        "DOSING_INFO",
        "FREQUENCY",
        "STUDYID",
        "STUDY_TYPE",
    ...
    if not invivo_df.empty:
        for rows in tqdm(
            invivo_df[COLS_invivo_df].values,
            desc="Populating graph with relations based on in-vivo df",
        ):
            (
                exp_id,
                cpd_id,
                ...
                pretreatment_dose,
                pretreatment_dosing_info,
                pretreatment_route_of_administration,
            ...
            if pd.notna(study_id) and pd.notna(animal_group_description):
                if animal_group_description != "NOT IN LIST (SEE COMMENT)":
                    _create_relation(
                        ...
                        relation_type="ASSOCIATED",
                        entity_b=node_mapping_dict["Animal group"][
                            animal_group_description
                        ],
                        tx=tx,
                    )
            ...
            if animal_group_description != "NOT IN LIST (SEE COMMENT)":
                if (
                    animal_group_description in node_mapping_dict["Animal group"]
                    and animal_number in node_mapping_dict["Animal number"]
                ):
                    _create_relation(
                        entity_a=node_mapping_dict["Animal group"][
                            animal_group_description
                        ],
                        relation_type="ASSOCIATED",
            ...
            if pd.notna(animal_group) and pd.notna(strain_name):
                if (
                    animal_group in node_mapping_dict["Animal group"]
                    and strain_name in node_mapping_dict["Bacteria"]
                ):
                    annotation = {}

                    if "PRETREATMENT_BATCH_ID" in invivo_df.columns and pd.notna(
                        pretreatment_batch_id
                    ):
                        annotation["pretreatment batch id"] = pretreatment_batch_id

                    if "PRETREATMENT_CPD_ID" in invivo_df.columns and pd.notna(
                        pretreatment_cpd_id
                    ):
                        annotation["pretreatment compound"] = pretreatment_cpd_id

                    if "PRETREATMENT_DOSE" in invivo_df.columns and pd.notna(
                        pretreatment_dose
                    ):
                        annotation["pretreatment dose (mg/kg)"] = pretreatment_dose

                    if "PRETREATMENT_DOSING_INFO" in invivo_df.columns and pd.notna(
                        pretreatment_dosing_info
                    ):
                        annotation["pretreatment dosing information"] = (
                            pretreatment_dosing_info
                        )

                    if (
                        "PRETREATMENT_ROUTE_OF_ADMINSTRATION" in invivo_df.columns
                        and pd.notna(pretreatment_route_of_administration)
                    ):
                        annotation["pretreatment route of administration"] = (
                            pretreatment_route_of_administration
                        )
                    ...
                    if pd.isna(infection_route_dict):
                        ...
                        if infection_route_dict != "":  # empty strings
                            infection_route_dict = ast.literal_eval(
                                infection_route_dict.replace("nan", "None")
                            )
                    ...
                    if pd.notna(roa_dict):
                        if not isinstance(roa_dict, dict):
                            if roa_dict != "":  # empty strings
                                roa_dict = ast.literal_eval(
                                    roa_dict.replace("nan", "None")
                                )
            ...
            if pd.notna(animal_number) and pd.notna(result_type):
                ...
                if pd.notna(method_dict):
                    if isinstance(method_dict, str) and method_dict != "":
                        method_dict = ast.literal_eval(method_dict)
                    method_dict = {
                        f"statistical_{i}": j for i, j in method_dict.items()
                    }
                ...
                if pd.notna(value) and pd.notna(operator) and "name" in unit_dict:
                    if unit_dict["name"] == "No unit":
                        ...
                        annotation["result"] = (
                            str(operator) + " " + str(value) + " " + unit_dict["name"]
                        )
            ...
            if (pd.notna(strain_name) and pd.notna(cpd_id)) and (
                strain_name in node_mapping_dict["Bacteria"]
            ):
                _create_relation(
            ...
            if pd.notna(exp_id) and pd.notna(result_type):
                ...
                if pd.notna(method_dict):
                    if isinstance(method_dict, str) and method_dict != "":
                        method_dict = ast.literal_eval(
                            method_dict.replace("null", "None")
                        )
                    method_dict = {
                        f"statistical_{i}": j for i, j in method_dict.items()
                    }
                ...
                annotation["ELN"] = (
                    f"https://www.sciencecloud.com/experiments/notebook/experiment/{exp_id}"
                )
    ...
    if not invitro_df.empty:
        for rows in tqdm(
            invitro_df[COLS_invitro_df].values,
            desc="Populating graph with in-vitro data",
        ):
            (
                exp_id,
                cpd_id,
            ...
            """Bacteria -> Compounds edge"""
            if (
                cpd_id in node_mapping_dict["Compound"]
                and strain_name in node_mapping_dict["Bacteria"]
            ):
                _create_relation(
                    entity_a=node_mapping_dict["Bacteria"][strain_name],
